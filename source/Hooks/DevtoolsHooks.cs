...
public static class DevtoolsHooks
{
    ...
    {
        orig(self, owner);
        if (owner.type == PlacedObjectType.HazerMom || owner.type == PlacedObjectType.AlbinoHazerMom || owner.type == PlacedObjectType.DeadHazerMom || owner.type == PlacedObjectType.DeadAlbinoHazerMom || owner.type == PlacedObjectType.DeadAlbinoFormHazer || owner.type == PlacedObjectType.AlbinoFormHazer || owner.type == PlacedObjectType.XyloNest || owner.type == PlacedObjectType.AltXyloNest || owner.type == PlacedObjectType.XyloWorm || owner.type == PlacedObjectType.BigXyloWorm || owner.type == PlacedObjectType.DeadXyloWorm || owner.type == PlacedObjectType.DeadBigXyloWorm)
        {
            self.minRegen = 7;
            self.maxRegen = 10;
        }
