	...
	Category
	{
		...
		SubShader
		{
			Pass
			{
				...
				sampler2D _MainTex;
				sampler2D _NoiseTex;
				uniform float4 _GRJMiniLeviathanColorA;
				uniform float4 _GRJMiniLeviathanColorB;
				uniform float4 _GRJMiniLeviathanColorHead;
				float4 _MainTex_ST;
				...
				float4 frag(v2f i) : SV_Target
				{
					...
					return lerp(_GRJMiniLeviathanColorHead, lerp(_GRJMiniLeviathanColorA, _GRJMiniLeviathanColorB, pow(i.uv.y, 2)), clamp((i.uv.y - 0.015) * 60.0, 0.0, 1.0));
				}
				ENDCG
			}
		} 
