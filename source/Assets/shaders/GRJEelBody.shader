	...
	Category
	{
		...
		SubShader
		{
			Pass
			{
				...
				sampler2D _MainTex;
				sampler2D _NoiseTex;
				uniform float4 _GRJLeviathanColorA;
				uniform float4 _GRJLeviathanColorB;
				uniform float4 _GRJLeviathanColorHead;
				float4 _MainTex_ST;
				...
				float4 frag(v2f i) : SV_Target
				{
					...
					return lerp(_GRJLeviathanColorHead, lerp(_GRJLeviathanColorA, _GRJLeviathanColorB, pow(i.uv.y, 2)), clamp((i.uv.y - 0.015) * 60.0, 0.0, 1.0));
				}
				ENDCG
			}
		} 
